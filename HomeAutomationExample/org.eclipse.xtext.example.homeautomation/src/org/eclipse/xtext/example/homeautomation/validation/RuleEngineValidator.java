...
public class RuleEngineValidator extends AbstractRuleEngineValidator {
	...
	public boolean isRecursive(XFeatureCall featureCall) {
		Rule containingRule = EcoreUtil2.getContainerOfType(featureCall, Rule.class);
		if (containingRule != null && featureCall.getFeature() instanceof JvmOperation
			...
			if (argument instanceof XAbstractFeatureCall) {
				EObject sourceElem = jvmModelAssociations.getPrimarySourceElement(((XAbstractFeatureCall) argument).getFeature());
				if (sourceElem == containingRule.getDeviceState()) {
					return true;
				}
			}
		}
